...
async function initSanity(args, context) {
  ...
  async function getOrCreateProject() {
    ...
    if (flags.organization) {
      ...
      if (!organization) {
        throw new Error(`Given organization ID (${flags.organization}) not found, or you do not have access to it`);
      }
  ...
  async function getOrganizationId(organizations) {
    ...
    if (shouldPrompt) {
      (0, _debug.default)(`User has ${organizations.length} organization(s), checking attach access`);
      const withGrant = await getOrganizationsWithAttachGrant(organizations);

      if (withGrant.length === 0) {
        (0, _debug.default)('User lacks project attach grant in all organizations, not prompting');
        return undefined;
      }

      (0, _debug.default)('User has attach access to %d organizations, prompting.', withGrant.length);
      ...
      (0, _debug.default)(`User has defined organization flag explicitly (%s)`, organizationId);
    } else if (organizations.length === 0) {
      (0, _debug.default)('User has no organizations, skipping selection prompt');
    }
