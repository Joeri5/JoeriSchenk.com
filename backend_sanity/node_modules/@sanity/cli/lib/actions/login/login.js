...
async function login(args, context) {
  ...
  const {
    sso,
    experimental
  } = args.extOptions; // _Potentially_ already authed client

  const authedClient = apiClient({
    requireUser: false,
    requireProject: false
  });
  const hasExistingToken = Boolean(authedClient.config().token); // Explicitly tell _this_ client not to use a token

  const client = authedClient.clone().config({
    token: undefined
